...
import { adminClassBookingService, type ClassBooking } from "@/services/classBooking.service";
...
const AdminClassBookings = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const res = await adminClassBookingService.list(params);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            All class bookings across the platform ({total} total)
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <BookOpen className="h-4 w-4" /> Class Bookings
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <p className="text-center py-12 text-gray-500">No class bookings found.</p>
          ) : (
            <>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {bookings.map((b) => (
                    <TableRow key={b.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{b.creditsCharged} cr</Badge>
                      </TableCell>
                      <TableCell>
